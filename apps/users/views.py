...
def calculate_late_minutes(event_datetime: datetime, user_work_time: time) -> int:
    ...
    work_datetime = datetime.combine(event_datetime.date(), user_work_time, event_datetime.tzinfo)

    # Calculate late minutes (only if event is after work time)
    late_minutes = max((event_datetime - work_datetime).total_seconds() // 60, 0)
